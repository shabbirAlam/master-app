...
* when modifying the existing snapshot test or writing new snapshot test then first record as `.all` and once screen is recorded, then change the record value to `.never` and check again for success.
